// src/pages/MigrationFormPage.jsx
...
import { fetchLegacyCaseDetail, migrateLegacyCase } from "../api/migrationApi";

const MigrationFormPage = () => {
  // Extract legacy case ID from route params
  ...
  const [legacyCase, setLegacyCase] = useState(null); // Store legacy case data
  ...
  // Load legacy case data and prefill form
  useEffect(() => {
    const loadLegacyCase = async () => {
      if (!legacyId) {
        setError("No legacy case ID provided");
        return;
      }

      try {
        setLegacyLoading(true);
        console.log("Fetching legacy case:", legacyId);
        
        const response = await fetchLegacyCaseDetail(legacyId);
        console.log("Legacy case response:", response);
        
        const legacyData = response.legacy_case || response;
        setLegacyCase(legacyData);
        
        // Prefill form data from legacy case
        setFormData((prev) => ({
          ...prev,
          // Text fields - map from legacy field names
          complaint_text: legacyData.complaint_content || legacyData.complaint_text || "",
          immediate_action: legacyData.immediate_action || "",
          taken_action: legacyData.actions_taken || legacyData.taken_action || "",
          
          // Org unit - prefill if valid
          issuing_department_id: legacyData.issuing_org_unit_id || legacyData.issuing_department_id || null,
          
          // Search fields - TEXT ONLY (no auto-select)
          // These will populate search input text, user must confirm selection
          patientQuery: legacyData.patient_name || "",
          doctorQuery: legacyData.doctor_name || "",
        }));
        
        console.log("✅ Legacy case prefilled:", {
          complaint_text: legacyData.complaint_content,
          immediate_action: legacyData.immediate_action,
          taken_action: legacyData.actions_taken,
          issuing_org_unit_id: legacyData.issuing_org_unit_id,
          patient_name: legacyData.patient_name,
          doctor_name: legacyData.doctor_name,
        });
        
      } catch (err) {
        console.error("Error loading legacy case:", err);
        setError(`Failed to load legacy case: ${err.message}`);
      } finally {
  ...
  const handleAddRecord = async () => {
    ...
    try {
      ...
      // Submit to backend - MIGRATION ENDPOINT (not normal insert)
      console.log(`🚀 Submitting migration for legacy case ${legacyId}`);
      const response = await migrateLegacyCase(legacyId, payload);

      console.log("Migration completed successfully:", response);

      // Show success message
      setSuccess(
        `✅ Migration completed! New case ID: ${response.case_id || response.id}`
      );
      setError(null);
      setErrorField(null);

      // Navigate back to migration list after success
      setTimeout(() => {
        navigate("/migration");
      }, 2000);
    } catch (err) {
      // Handle error response format from backend - DO NOT RESET FORM
      const errorMessage = err.message_ar || err.message || "Error during migration. Please try again.";
  ...
  return (
    <MainLayout>
      <Container maxWidth="lg" sx={{ py: 3 }}>
        <Box sx={{ mb: 3 }}>
          <h1 style={{ color: "#1a1e3f", marginBottom: "8px" }}>
            Migration Form — Legacy Case #{legacyId}
          </h1>
        </Box>
...
export default MigrationFormPage;
